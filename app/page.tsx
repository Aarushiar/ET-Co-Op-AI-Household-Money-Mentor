...
export default function Home() {
  ...
  const [isDemoBooting, setIsDemoBooting] = useState(false);
  ...
  async function enterLiveDemo() {
    setIsDemoBooting(true);
    applyScenario("dual-income-urban");

    const demoEmail = `demo+${Date.now()}@jointtaxterminal.ai`;
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error ?? "Unable to boot demo session.");
      }

      setCurrentUserEmail(data.userEmail ?? demoEmail);
      setIsAuthenticated(true);
    } catch {
      // Keep the demo deterministic even if network/session bootstrap fails.
      setCurrentUserEmail("demo@jointtaxterminal.ai");
      setIsAuthenticated(true);
    } finally {
      setIsAuthLoading(false);
      setAuthEmail("");
      ...
      setAuthErrorMessage(null);
      setAuthMode("signin");
      setIsDemoBooting(false);
      setToastMessage("Live demo unlocked. Joint Tax Terminal is ready.");
    }
  }
      ...
      <AnimatePresence>
        {toastMessage ? (
          <motion.div
            ...
            className="fixed right-4 top-4 z-50 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800 shadow-[0_14px_36px_rgba(16,185,129,0.25)] dark:border-emerald-500/40 dark:bg-emerald-500/15 dark:text-emerald-200"
          >
            ...
            <div className="mt-4 max-w-md space-y-3">
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                <button
                  ...
                  disabled={isAuthSubmitting || isDemoBooting}
                  className="sota-focus-ring inline-flex h-11 w-full items-center justify-center rounded-xl border border-blue-700 bg-blue-700 px-5 text-sm font-semibold text-white transition hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {isAuthSubmitting
                    ? authMode === "signin"
                      ? "Signing in..."
                ...
                <button
                  ...
                  disabled={isAuthSubmitting || isDemoBooting}
                  className="sota-focus-ring inline-flex h-11 w-full items-center justify-center rounded-xl border border-emerald-700 bg-gradient-to-r from-emerald-700 to-emerald-600 px-5 text-sm font-semibold text-white shadow-[0_10px_24px_rgba(5,150,105,0.24)] transition hover:from-emerald-600 hover:to-emerald-500"
                >
                  {isDemoBooting ? "Launching Demo..." : "Try Live Demo"}
                </button>
              </div>
